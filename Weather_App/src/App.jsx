...
const App = () => {
  ...
  useEffect(() => {
    if (city.trim().length >= 3 && !weather) {
      const timer = setTimeout(() => fetchSuggestation(city), 500);
      return () => clearTimeout(timer);
    }
    setSuggest([]);
  }, [city, weather]);

  const fetchSuggestation = async (query) => {
    try {
      const res = await fetch(
        `http://api.openweathermap.org/geo/1.0/direct?q=${query}&limit=5&appid=${API_Key}`
      );
      res.ok ? setSuggest(await res.json()) : setSuggest([]);
    } catch (error) {
      setSuggest([]);
    }
  };
  ...
  //this function will validate and prevents city to be null value and fetches data via api
          ...
          {!weather ? (
            ...
            <div className="mt-6 text-center transition-opacity duration-500">
              <button
                ...
                className="mb-4 bg-purple-900 hover:bg-blue-700 text-white font-semibold py-1 px-3 rounded transition-colors cursor-pointer "
              >
                New Search
              </button>
              ...
              <div className="flex flex-wrap justify-around mt-6">
                {[
                  ...
                  <div key={label} className="flex flex-col items-center m-2">
                    <Icon />
                    <p className="mt-1 font-semibold">{label}</p>
                    <p className="text-sm">{new Date(time * 1000).toLocaleDateString('en-Us',
